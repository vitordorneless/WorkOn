...
<script>
    $(document).ready(function () {
        ...
        function envia_form() {
            ...
            if ($("#status").val() === 'na')
            {
                $("#status_error").html("<div class='alert alert-danger alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>×</button>Informe Status...</div>"),
                        $("#status").focus();
                return false;
            } else {
                $("#status_error").empty();
            ...
            $.ajax({
                type: "POST",
                dataType: "HTML",
                url: "../Controller/herval_agendamento_editar.php",
                data: "unidade=" + unidade + "&data_agendamento=" + data_agendamento + "&situacao=" + situacao + "&medico=" + medico +
                        "&consulta=" + consulta + "&id_tipo_agendamento=" + id_tipo_agendamento + "&id=" + id + "&status=" + status,
                beforeSend: function () {
...
<div class="modal-header">
    <h2><strong>Editar</strong> Agendamento Herval</h2>
        ...
        <form id="form" method="POST">
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    ?>
                    <input type="date" class="form-control" id="data_agendamento" name="data_agendamento" value="<?php echo $date_solicitacao; ?>">
                </div>
            ...
            <div class="form-group">
                <label for="label_consulta">Valor da Consulta:</label>                    
                    <input type="text" class="form-control" id="consulta" name="consulta" placeholder="Informe Valor Acertado" value="<?php echo $array_herval['valor_consulta']; ?>">
            ...
            <button class="btn btn-primary btn-vimeo pull-right" id="envia" type="submit">Editar Agendamento Herval <span class="glyphicon glyphicon-floppy-save" aria-hidden="true"></span></button>
        </form>
    </div>
</div>
