<div id="refresca_cassi_solicitante">
    ...
    <div class="row">
        <div class="col-md-8">
            <a href="demandas_admin_tipo_demandas_incluir.php" data-toggle="modal" data-target="#myModal1" class="btn btn-default btn-dropbox">Incluir Tipo de Demanda <span class="glyphicon glyphicon-save" aria-hidden="true"></span></a><br><br>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="widget">
                <div class="widget-header">
                    <h2><strong>Listar </strong>Tipos de Demandas</h2>
                        ...
                        <form class='form-horizontal' role='form'>
                            <table id="funcionarios" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    $querie = new Queries();
                                    $sla = new Demandas_Prazos();                                    
                                    foreach ($pdo->query($querie->demanda_admin_listar_tipos()) as $value) {
                                        $prazo = $sla->Dados_Demandas($value['sla']);                                        
                                        $situation = $value['status'] == 1 ? "Ativo" : "Inativo";
                                        echo '<tr>';
                                        echo '<td>' . $value['tipo_demanda'] . '</td>';
                                        echo '<td>' . $prazo['prazo'] . ' '.$prazo['tipo'].'</td>';
                                        echo '<td>' . $value['setor'] . '</td>';
                                        echo '<td>' . $situation . '</td>';
                                        echo '<td><a href="demandas_admin_tipo_demandas_editar.php?id=' . $value['id'] . '" data-toggle="modal" data-target="#myModal" class="btn btn-default btn-dropbox">Editar <span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a></td>';
                                        echo '</tr>';
                                    }
                                    Database::disconnect();
                                    ?>
