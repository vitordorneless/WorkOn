                        ...
                        <form class='form-horizontal' role='form'>
                            <table id="funcionarios" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    $sql = "select id, nome_funcionario, cod_empresa, cod_estabelecimento, cpf from wal_funcionarios where periodo in (2015) and erro in (1) order by cod_empresa, cod_estabelecimento, nome_funcionario asc";
                                    foreach ($pdo->query($sql) as $value) {

                                        $sql_unidade = 'select desc_empresa from wal_empresa where cod_empresa in (' . $value['cod_empresa'] . ')';
                                        $qq = $pdo->prepare($sql_unidade);
                                        $qq->execute();
                                        $data_unidade = $qq->fetch(PDO::FETCH_ASSOC);

                                        $sql_sit = 'select desc_estabelecimento from wal_estabelecimento where cod_empresa in (' . $value['cod_empresa'] . ') and cod_estabelecimento in (' . $value['cod_estabelecimento'] . ')';
