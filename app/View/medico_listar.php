                        ...
                        <form class='form-horizontal' role='form'>
                            <table id="funcionarios" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    $sql = "select doctors.id_medico as id_medico, doctors.nome as nome, doctors.crm as crm, 
                                            function_doctors.funcao as funcao, doctors.cod_sig as cod_sig, 
                                            doctors.ddd_telefone as ddd_telefone, doctors.telefone as telefone, doctors.status as status
                                            from wal_medico doctors
                                            inner join wal_funcao_medico function_doctors on function_doctors.id_funcao = doctors.id_funcao
                                            where doctors.status = 1
                                            order by doctors.crm";
                                    foreach ($pdo->query($sql) as $value) {
                                        echo '<tr>';
                                        echo '<td>' . utf8_encode($value['nome']) . '</td>';                                        
                                        echo '<td>' . $value['crm'] . '</td>';
                                        echo '<td>' . $value['funcao'] . '</td>';
                                        echo '<td>' . $value['cod_sig'] . '</td>';
                                        echo '<td>' . $value['ddd_telefone'].'-'.$value['telefone'] . '</td>';
                                        $situation = $value['status'] == 1 ? "Ativo" : "Inativo";
                                        echo '<td>' . $situation . '</td>';
                                        echo '<td><a href="medico_editar.php?id_medico=' . $value['id_medico'] . '" data-toggle="modal" data-target="#myModal" class="btn btn-default">Editar <span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a></td>';
                                        echo '</tr>';
                                    }
                                    Database::disconnect();
                                    ?>
