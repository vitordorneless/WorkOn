        ...
        <div class="col-md-12">
            <div class="widget">
                <div class="widget-header">
                    <h2><strong>Enviar </strong>Agendamentos Herval por Email</h2>                    
                            ...
                            <table id="funcionarios" class="display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    foreach ($pdo->query($sql) as $value) {
                                        ...
                                        $sql_email = 'select count(id) as temos from herval_email_agendamento where id_agendamento in ('.$value['id'].')';
                                        $qqqqs = $pdo->prepare($sql_email);
                                        $qqqqs->execute();
                                        $datim = $qqqqs->fetch(PDO::FETCH_ASSOC);
                                        $enviou = $datim['temos'] > 0 ? "Sim" : "Ainda Não";
                                        echo '<tr>';
                                        ...
                                        echo '<td><strong>' . $enviou . '</strong></td>';
                                        echo '<td>' . utf8_encode($data_cargo['situacao']) . '</td>';
                                        echo '<td><a href="herval_agendamento_enviar_email.php?id=' . $value['id'] . '" data-toggle="modal" data-target="#myModal" class="btn btn-default btn-danger">Enviar Email Agendamento <span class="glyphicon glyphicon-envelope" aria-hidden="true"></span></a></td>';
                                        echo '</tr>';
                                    }
                                    Database::disconnect();
                                    ?>
