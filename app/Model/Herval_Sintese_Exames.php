...
class Herval_Sintese_Exames extends Herval {

    public function save_Herval_sint_body($id_herval_sinteses_cab, $id_herval_setor,$id_herval_funcao,$agente_fisico, $obs_agente_fisico, $agente_quimico, $obs_agente_quimico, $agente_biologico, $obs_agente_biologico, $agente_ergonomico
    ...
    , $plaquetas, $eritograma, $acido_tt_muconico, $glicemia_em_jejum, $avaliacao_psicossocial, $acido_hipurico, $obs_1, $obs_2, $obs_3, 
            $obs_4, $obs_5, $obs_6, $obs_7, $obs_8) {
        include_once '../config/database_mysql.php';
        $status = 1;
        $data_ultima_alteracao = date('Y-m-d H:i:s');
        $pdo = Database::connect();
        $smtp = $pdo->prepare("INSERT INTO herval_sintese_conteudo(id_herval_sinteses_cab,id_herval_setor,id_herval_funcao,agente_fisico,obs_agente_fisico,
                ...
                acido_hipurico,obs_1,obs_2,obs_3,obs_4,obs_5,obs_6,obs_7,obs_8,status,data_ultima_alteracao) 
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $smtp->bindParam(1, $id_herval_sinteses_cab, PDO::PARAM_INT);
        ...
        $smtp->bindParam(42, $status, PDO::PARAM_INT);
        $smtp->bindParam(43, $data_ultima_alteracao, PDO::PARAM_STR);        
    ...
    public function edit_Herval_sint_body($id, $id_herval_sinteses_cab, $id_herval_setor,$id_herval_funcao,$agente_fisico, $obs_agente_fisico, $agente_quimico, $obs_agente_quimico, $agente_biologico, $obs_agente_biologico, $agente_ergonomico
    , $obs_agente_ergonomico, $ausencia_de_risco, $obs_ausencia_de_risco, $exame_clinico, $acido_metil_hipurico, $hemograma, $acido_mandelico, $vdrl
    , $reticulocitos, $parasitologico_fezes, $cultural_de_orofaringe, $coprocultura, $micologico_de_unha, $audiometria, $ecg, $acuidade_visual, $eeg
    , $plaquetas, $eritograma, $acido_tt_muconico, $glicemia_em_jejum, $avaliacao_psicossocial, $acido_hipurico, $obs_1, $obs_2, $obs_3, $obs_4, $obs_5, $obs_6, $obs_7, $obs_8) {
        include_once '../config/database_mysql.php';
        $status = 1;
        $data_ultima_alteracao = date('Y-m-d H:i:s');
        $pdo = Database::connect();
        $smtpup = $pdo->prepare("UPDATE herval_sintese_conteudo SET id_herval_sinteses_cab = :id_herval_sinteses_cab,id_herval_setor = :id_herval_setor,id_herval_funcao = :id_herval_funcao,agente_fisico = :agente_fisico,obs_agente_fisico = :obs_agente_fisico,
                ...
                acido_hipurico = :acido_hipurico,obs_1 = :obs_1,obs_2 = :obs_2,obs_3 = :obs_3,obs_4 = :obs_4,obs_5 = :obs_5,obs_6 = :obs_6,obs_7 = :obs_7,obs_8 = :obs_8,status = :status,data_ultima_alteracao = :data_ultima_alteracao WHERE id = :id");
        $confirm = $smtpup->execute(array(
            ':id' => $id,
            ':id_herval_sinteses_cab' => $id_herval_sinteses_cab,
    ...
    public function delete_Herval_sint_body($id) {
        include_once '../config/database_mysql.php';
        $status = 0;
        $data_ultima_alteracao = date('Y-m-d H:i:s');
        $pdo = Database::connect();
        $smtpup = $pdo->prepare("UPDATE herval_sintese_conteudo SET status = :status, data_ultima_alteracao = :data_ultima_alteracao WHERE id = :id");
        $confirm = $smtpup->execute(array(
            ':id' => $id,
            ':status' => $status,
            ':data_ultima_alteracao' => $data_ultima_alteracao));
    ...
    public function Dados_Herval_sint_bodys($id) {
        include_once '../config/database_mysql.php';
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "select id,id_herval_sinteses_cab,id_herval_setor,id_herval_funcao,agente_fisico,obs_agente_fisico,
                agente_quimico,obs_agente_quimico,agente_biologico,obs_agente_biologico,agente_ergonomico,obs_agente_ergonomico,
                ausencia_de_risco,obs_ausencia_de_risco,exame_clinico,acido_metil_hipurico,hemograma,acido_mandelico,vdrl,
                reticulocitos,parasitologico_fezes,cultural_de_orofaringe,coprocultura,micologico_de_unha,audiometria,ecg,
                acuidade_visual,eeg,plaquetas,eritrograma,acido_tt_muconico,glicemia_em_jejum,avaliacao_psicossocial,
                acido_hipurico,obs_1,obs_2,obs_3,obs_4,obs_5,obs_6,obs_7,obs_8,status,data_ultima_alteracao from herval_sintese_conteudo where id_herval_sinteses_cab = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
