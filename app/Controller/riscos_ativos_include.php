                        ...
                        <table id="funcionarios" class="table table-responsive tab-boxed table-bordered table-striped" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                foreach ($pdo->query($sql) as $value) {
                                    ...
                                    $sql4 = 'select count(id) as temos from wal_funcionarios_exames where ativo = 1 and id_exame = 21 and id_funcionario = ' . $value['id'];
                                    $q4 = $pdo->prepare($sql4);
                                    $q4->execute();
                                    $data4 = $q4->fetch(PDO::FETCH_ASSOC);
                                    $apto = $data4['temos'] > 0 ? "Sim" : "Não";

                                    $sql5 = 'select count(id) as temos from wal_apto_trab_altura where status = 1 and id_funcionario = ' . $value['id'];
                                    $q5 = $pdo->prepare($sql5);
                                    $q5->execute();
                                    ...
                                    if (($apto == "Sim") or ( $apto1 === "Sim")) {
                                        echo '<td><strong>Vá para a tela <br>Editar Risco por Associado!!</strong></td>';
                                    } else {
                                        echo '<td><a href="#" id="add_riscos.php?id=' . $value['id'] . '" class="btn btn-small btn-facebook">Add Risco Individual <span class="glyphicon glyphicon-paperclip" aria-hidden="true"></span></a></td>';
                                    }
                                    echo '</tr>';
                                }
                                Database::disconnect();
