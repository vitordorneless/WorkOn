...
$nome_txt = $_SESSION['user_id'] . '_' . $txt->get_empresa() . '_' . $txt->get_estabelecimento() . '_' . date('dmY') . '.txt';
fopen("../../uploads/grafica/" . $nome_txt, "a");

$array_coordenador_PCMSO = $grafica->Dados_Medico_Coordenador($txt->get_funcao());
$codigo_empresa = str_replace('x', ' ', str_pad("", 3, 'x', STR_PAD_RIGHT));
$razao_social = str_replace('x', ' ', str_pad("", 50, 'x', STR_PAD_RIGHT));
$unidade = str_pad("", 6, 'x', STR_PAD_RIGHT);
$matricula = str_replace('x', ' ', str_pad("", 8, 'x', STR_PAD_RIGHT));
$nome = str_pad($array_coordenador_PCMSO['nome'], 50);
...
$sql = 'select funfun.id as id_funcionario, funfun.nome_funcionario as nome_ativo,funfun.matricula as matricula, funfun.cod_depto as departamento, depdep.desc_depto as nome_setor,
        funfun.identidade as rg, funfun.cpf as cpf, funfun.nascimento as nascimento,funfun.cod_empresa as cod_empresa, funfun.cod_estabelecimento as loja, funfun.cod_cargo as cargo, funfun.cod_centro_custo as setor  
        from wal_funcionarios funfun
        inner join wal_departamento depdep on depdep.cod_depto = funfun.cod_depto
        where funfun.cod_empresa = ' . $txt->get_empresa() . ' and funfun.cod_estabelecimento = ' . $txt->get_estabelecimento() . '
        and funfun.exame = 1 and funfun.risco = 1 and nascimento <> "" 
...
foreach ($pdo->query($sql) as $value) {
    ...
    $data_nascimentos = $value['nascimento'] == NULL ? '0' : $value['nascimento'];
    $data_nascimento = str_replace('/', '', str_pad($data_nascimentos,8));
    $qrcode = str_pad("http://www.grupoamasaude.com.br/aso/?cpf=" . $value['cpf'], 450);

    $sql_loja = 'select desc_estabelecimento from wal_estabelecimento where cod_empresa = ' . $empresa . ' and cod_estabelecimento = ' . $loja_parametro;
    $q = $pdo->prepare($sql_loja);
    $q->execute();
    $data_loja = $q->fetch(PDO::FETCH_ASSOC);

    $sql_cargo = 'select desc_cargo from wal_cargo where cod_cargo = ' . $id_cargo . ' limit 1';
    ...
    $sql_exames_riscos = "select 
                    ...
                    (select distinct obs_risco from wal_funcionarios_riscos where id_funcionario = $id_funcionario and id_risco = 1 limit 1) as obs_agente_fisico,
                    ...
                    (select distinct obs_risco from wal_funcionarios_riscos where id_funcionario = $id_funcionario and id_risco = 2 limit 1) as obs_agente_quimico,
                    ...
                    (select distinct obs_risco from wal_funcionarios_riscos where id_funcionario = $id_funcionario and id_risco = 3 limit 1) as obs_agente_biologico,
                    ...
                    (select distinct obs_risco from wal_funcionarios_riscos where id_funcionario = $id_funcionario and id_risco = 4 limit 1) as obs_agente_ergonomico,
                    ...
                    (select distinct obs_risco from wal_funcionarios_riscos where id_funcionario = $id_funcionario and id_risco = 5 limit 1) as obs_ausencia_de_risco,
    ...
    $sql_apto_altura = 'select count(id) as temos from wal_apto_trab_altura where status = 1 and id_funcionario = ' . $id_funcionario;
    $p = $pdo->prepare($sql_apto_altura);
    $p->execute();
    $data_apto = $p->fetch(PDO::FETCH_ASSOC);

    $marcaAgenteOUTROS = $data_exames_e_riscos['trab_altura'] == 1 ? 'x' : '';
    $apto_altura = $data_apto['temos'] > 0 ? "APTO PRA TRABALHO EM ALTURA" : "";
    $DescAgenteOUTROS = $data_exames_e_riscos['trab_altura'] == 1 ? str_pad("Trabalho em Altura", 100) : str_pad("", 100);
    $agente_fisico = $data_exames_e_riscos['agente_fisico'] == 1 ? 'x' : '';
    $agente_quimico = $data_exames_e_riscos['agente_quimico'] == 1 ? 'x' : '';
    $agente_biologico = $data_exames_e_riscos['agente_biologico'] == 1 ? 'x' : '';
    $agente_ergonomico = $data_exames_e_riscos['agente_ergonomico'] == 1 ? 'x' : '';
    $ausencia_de_risco = $data_exames_e_riscos['ausencia_de_risco'] == 1 ? 'x' : '';
    $exame_clinico = $data_exames_e_riscos['exame_clinico'] == 1 ? 'x' : '';
    $acido_metil_hipurico = $data_exames_e_riscos['acido_metil_hipurico'] == 1 ? 'x' : '';
    $hemograma = $data_exames_e_riscos['hemograma'] == 1 ? 'x' : '';
    $acido_mandelico = $data_exames_e_riscos['acido_mandelico'] == 1 ? 'x' : '';
    $vdrl = $data_exames_e_riscos['vdrl'] == 1 ? 'x' : '';
    $reticulocitos = $data_exames_e_riscos['reticulocitos'] == 1 ? 'x' : '';
    $parasitologico_fezes = $data_exames_e_riscos['parasitologico_fezes'] == 1 ? 'x' : '';
    $cultural_de_orofaringe = $data_exames_e_riscos['cultural_de_orofaringe'] == 1 ? 'x' : '';
    $coprocultura = $data_exames_e_riscos['coprocultura'] == 1 ? 'x' : '';
    $micologico_de_unha = $data_exames_e_riscos['micologico_de_unha'] == 1 ? 'x' : '';
    $audiometria = $data_exames_e_riscos['audiometria'] == 1 ? 'x' : '';
    $ecg = $data_exames_e_riscos['ecg'] == 1 ? 'x' : '';
    $acuidade_visual = $data_exames_e_riscos['acuidade_visual'] == 1 ? 'x' : '';
    $eeg = $data_exames_e_riscos['eeg'] == 1 ? 'x' : '';
    $plaquetas = $data_exames_e_riscos['plaquetas'] == 1 ? 'x' : '';
    $eritrograma = $data_exames_e_riscos['eritrograma'] == 1 ? 'x' : '';
    $acido_tt_muconico = $data_exames_e_riscos['acido_tt_muconico'] == 1 ? 'x' : '';
    $glicemia_em_jejum = $data_exames_e_riscos['glicemia_em_jejum'] == 1 ? 'x' : '';
    $acido_hipurico = $data_exames_e_riscos['acido_hipurico'] == 1 ? 'x' : '';
    $avPsicossocial = $data_exames_e_riscos['avaliacao_psicossocial'] == 1 ? '' : '';
    $Psicossocial = str_pad(" ", 20);
    $obs_agente_fisico = $data_exames_e_riscos['obs_agente_fisico'] == NULL ? " " : str_pad($data_exames_e_riscos['obs_agente_fisico'], 100);
    $obs_agente_quimico = $data_exames_e_riscos['obs_agente_quimico'] == NULL ? " " : str_pad($data_exames_e_riscos['obs_agente_quimico'], 100);
    $obs_agente_biologico = $data_exames_e_riscos['obs_agente_biologico'] == NULL ? " " : str_pad($data_exames_e_riscos['obs_agente_biologico'], 100);
    $obs_agente_ergonomico = $data_exames_e_riscos['obs_agente_ergonomico'] == NULL ? " " : str_pad($data_exames_e_riscos['obs_agente_ergonomico'], 100);
    $obs_ausencia_de_risco = $data_exames_e_riscos['obs_ausencia_de_risco'] == NULL ? " " : str_pad($data_exames_e_riscos['obs_ausencia_de_risco'], 100);

    $descUnidade = str_pad($data_loja['desc_estabelecimento'], 50);
    $nome_ativo = str_pad($value['nome_ativo'], 50);
    $rg = str_pad($value['rg'], 20);
    $descFuncao = str_pad($data_cargo['desc_cargo'], 50);
    $cpf = str_pad($value['cpf'], 20);
    ...
    $QuadroAptoComplementar = str_pad($apto_altura, 100);
    ...
    $linha_txt = $codigo_empresa . $razao_social . $unidade . $descUnidade . $matricula . $nome_ativo . $rg .
            ...
            $avPsicossocial . $Psicossocial . $paraExerceraFuncao . $descricaoParaExerceraFuncao . $quebralinha;
