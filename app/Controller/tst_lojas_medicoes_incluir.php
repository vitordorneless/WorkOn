...
$confirm = $tst->save_TST_checklist_Medicao($tst->get_id_unidade(), $tst->get_setor(), $tst->get_funcao(), $tst->get_db(), $tst->get_lux());
$confirm == TRUE ? $log->save_TST_Log('Inclusão de Medição na loja '.$tst->get_id_unidade(), $_SESSION['user_id'], date('Y-m-d H:i:s'), 1) : $log->save_TST_Log('Inclusão de Medição com erro na loja '.$tst->get_id_unidade(), $_SESSION['user_id'], date('Y-m-d H:i:s'), 0);
