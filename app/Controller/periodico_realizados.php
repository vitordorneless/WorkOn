...
if ($periodico != '') {
    ...
    if (($erro_identificacao != '') and ($erro_duplicado != '')) {
        $sql22 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_identificacao = 1, data_ultima_alteracao = now() where id in($erro_identificacao)";
        $executa22 = $pdo->prepare($sql22);
        $executa22->execute();
    }
    
    if (($erro_riscos != '') and ($erro_duplicado != '')) {
        $sql33 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_riscos = 1, data_ultima_alteracao = now() where id in($erro_riscos)";
        $executa33 = $pdo->prepare($sql33);
        $executa33->execute();
    }
    
    if (($erro_data_ASO != '') and ($erro_duplicado != '')) {
        $sql44 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_data_ASO = 1, data_ultima_alteracao = now() where id in($erro_data_ASO)";
        $executa44 = $pdo->prepare($sql44);
        $executa44->execute();
    }
    
    if (($erro_data_exames != '') and ($erro_duplicado != '')) {
        $sql55 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_data_exames = 1, data_ultima_alteracao = now() where id in($erro_data_exames)";
        $executa55 = $pdo->prepare($sql55);
        $executa55->execute();
    }
    
    if (($erro_assinatura_ativo != '') and ($erro_duplicado != '')) {
        $sql66 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_assinatura_ativo = 1, data_ultima_alteracao = now() where id in($erro_assinatura_ativo)";
        $executa66 = $pdo->prepare($sql66);
        $executa66->execute();
    }
    
    if (($erro_assinatura_medico != '') and ($erro_duplicado != '')) {
        $sql77 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_assinatura_medico = 1, data_ultima_alteracao = now() where id in($erro_assinatura_medico)";
        $executa77 = $pdo->prepare($sql77);
        $executa77->execute();
    }
    
    if (($erro_carimbo != '') and ($erro_duplicado != '')) {
        $sql88 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_carimbo = 1, data_ultima_alteracao = now() where id in($erro_carimbo)";
        $executa88 = $pdo->prepare($sql88);
        $executa88->execute();
    }
    
    if (($erro_rasuras != '') and ($erro_duplicado != '')) {
        $sql99 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_rasuras = 1, data_ultima_alteracao = now() where id in($erro_rasuras)";
        $executa99 = $pdo->prepare($sql99);
        $executa99->execute();
    }
    
    if (($erro_falta_apto != '') and ($erro_duplicado != '')) {
        $sql100 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_falta_apto = 1, data_ultima_alteracao = now() where id in($erro_falta_apto)";
        $executa100 = $pdo->prepare($sql100);
        $executa100->execute();
    }
    
    if (($erro_falta_habilitado != '') and ($erro_duplicado != '')) {
        $sql111 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_falta_habilitado = 1, data_ultima_alteracao = now() where id in($erro_falta_habilitado)";
        $executa111 = $pdo->prepare($sql111);
        $executa111->execute();
    }
    
    if (($erro_tel != '') and ($erro_duplicado != '')) {
        $sql122 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_tel = 1, data_ultima_alteracao = now() where id in($erro_tel)";
        $executa122 = $pdo->prepare($sql122);
        $executa122->execute();
    }
    
    if (($erro_coord != '') and ($erro_duplicado != '')) {
        $sql133 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_coord = 1, data_ultima_alteracao = now() where id in($erro_coord)";
        $executa133 = $pdo->prepare($sql133);
        $executa133->execute();
    }
    
    if (($falta_aso != '') and ($erro_duplicado != '')) {
        $sql133 = "UPDATE wal_funcionarios SET flg_periodico = 1, erro = 1, erro_falta_aso = 1, data_ultima_alteracao = now() where id in($falta_aso)";
        $executa133 = $pdo->prepare($sql133);
        $executa133->execute();
    }
    ...
    if ($executa_sql) {
        echo '<div class="alert alert-success alert-dismissable">
          <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
            Periódicos marcados com Sucesso!!</div>';
    } else {
