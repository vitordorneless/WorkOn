...
id_ACIDO_TT_MUCONICO,comp_GLICEMIA_EM_JEJUM,id_GLICEMIA_EM_JEJUM,comp_ACIDO_HIPURICO,id_ACIDO_HIPURICO,comp_AVALIACAO_PSICOSSOCIAL,id_AVALIACAO_PSICOSSOCIAL
FROM wal_funcionarios where flg_periodico in (1) and cod_pessoa_fisica not in (0) 
...
foreach ($pdo->query($sql) as $value) {
    ...
    $sql_unidade = "select desc_estabelecimento, cod_estabelecimento, cnpj from wal_estabelecimento where cod_empresa in (" . $value['cod_empresa'] . ") and cod_estabelecimento in (" . $value['cod_estabelecimento'] . ")";
    $q = $pdo->prepare($sql_unidade);
    $q->execute();
    $data_unidade = $q->fetch(PDO::FETCH_ASSOC);    

    $sql_setor = "select desc_depto from wal_departamento where cod_depto in (" . $value['cod_depto'] . ")";
    $qqq = $pdo->prepare($sql_setor);
    $qqq->execute();
    $data_setor = $qqq->fetch(PDO::FETCH_ASSOC);

    $sql_cargo = "select desc_cargo from wal_cargo where cod_cargo in ('" . $value['cod_cargo'] . "')";
