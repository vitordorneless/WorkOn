...
$confirm = $tst->edit_TST_Agendamento($tst->get_id(), $tst->get_id_tipo_agendamento(), $tst->get_id_unidade(), $tst->get_id_turnos(), $tst->get_id_situacao(), $tst->get_data_tarefa(), $tst->get_id_tecnicos(), $tst->get_obs(), $tst->get_status());
$confirm == TRUE ? $log->save_TST_Log('Edição de Agendamento ' . $tst->get_id_unidade(), $_SESSION['user_id'], date('Y-m-d H:i:s'), 1) : $log->save_TST_Log('Edição de Agendamento ' . $tst->get_id_unidade(), date('Y-m-d H:i:s'), 0);
echo $confirm == TRUE ? '<div class="alert alert-success" role="alert">Agenda Editada!</div>' : '<div class="alert alert-danger" role="alert">Errou!</div>';