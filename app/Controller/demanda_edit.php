...
$demanda->set_id_f(filter_input(INPUT_POST, 'id', FILTER_SANITIZE_NUMBER_INT));
...
$demanda->set_data_ultima_alteracao(filter_input(INPUT_POST, 'dt', FILTER_SANITIZE_STRING));
...
$voucher = date('Y').'0'.$demanda->get_id_f();
$confirm = $demanda->editDemanda($demanda->get_id_f(),$demanda->get_id(), $demanda->get_setor(), $demanda->get_destinosetor(), $demanda->get_id_responsavel(), $demanda->get_executantes(), $demanda->get_id_demanda(), $demanda->get_copyemail(), $demanda->get_prazo(), $demanda->get_status(),$demanda->get_data_ultima_alteracao());
$body = '<div class="containerr">';
$body = $body . '<h3>Demanda: ' . $array_demanda['tipo_demanda'] . ' Editada</h3>';
...
$subject = $voucher . ' ' . $array_demanda['tipo_demanda']. ' <<Editada>>';
if ($confirm === TRUE) {
    ...
    echo '<div class="alert alert-success alert-dismissable">
          ...
          Editado com Sucesso!!<br>ID: '.$voucher.'</div>';
} else {
